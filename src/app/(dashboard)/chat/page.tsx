...
export default function ChatPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-5.5rem)] space-y-3 pb-2">
      {/* Main Two-Column Chat Container */}
      <div className="flex-1 flex bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xs">
        {/* LEFT COLUMN: Conversation List */}
        <div
          className={`w-full md:w-80 lg:w-96 flex flex-col border-r border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/50 ${
            activeConversationId ? "hidden md:flex" : "flex"
          }`}
        >
          {/* Header & New Chat Button */}
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                  ...
                  />
                  <span>{isConnected ? `${onlineUserIds.size} Online` : "Connecting..."}</span>
                </div>
              </div>
            </div>
          ...
          {/* Filter Pills */}
          <div className="flex p-2 gap-1 border-b border-slate-200/80 dark:border-slate-800/80 bg-slate-100/70 dark:bg-slate-950/40 text-xs">
          ...
          {/* Search Box */}
          <div className="p-3 border-b border-slate-200/70 dark:border-slate-800/60 bg-white/40 dark:bg-transparent">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {loadingConversations ? (
              ...
              <div className="text-center py-12 px-4">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">No conversations found.</p>
                <button
                  onClick={() => setIsNewChatOpen(true)}
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-3 rounded-xl transition-all text-left cursor-pointer ${
                      isSelected
                        ? "bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800/80 text-slate-900 dark:text-slate-100 shadow-xs"
                        : "hover:bg-slate-100 dark:hover:bg-slate-800/50 border border-transparent text-slate-700 dark:text-slate-300"
                    }`}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="relative shrink-0">
                        {isDirect ? (
                          <div className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 flex items-center justify-center text-sm font-bold text-slate-700 dark:text-slate-200 overflow-hidden">
                            {otherUser?.avatarUrl ? (
                          ...
                          <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-800/70 flex items-center justify-center text-indigo-600 dark:text-indigo-400">
                    ...
                    {conv.unreadCount > 0 && (
                      <span className="ml-2 px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-600 text-white shrink-0 shadow-xs animate-pulse">
                        {conv.unreadCount}
                      </span>
                    )}
                  </button>
        ...
        >
          {activeConversation ? (
            <>
              ...
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 backdrop-blur-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative">
                    {activeConversation.type === "DIRECT" ? (
                      <div className="w-9 h-9 rounded-full bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 flex items-center justify-center text-xs font-bold text-slate-700 dark:text-slate-200 overflow-hidden">
                      ...
                      <div className="w-9 h-9 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800/80 flex items-center justify-center text-indigo-600 dark:text-indigo-400">
                        {activeConversation.type === "PROJECT" ? <FolderKanban className="w-4 h-4" /> : <Users className="w-4 h-4" />}
                      </div>
                    )}
                  </div>
                ...
                {/* Right Header Actions (Project Link + Cross Close Sign) */}
                <div className="flex items-center gap-2">
                  {activeConversation.project && (
                    <button
                      onClick={() => router.push(`/projects/${activeConversation.project.id}`)}
                      className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold transition-colors cursor-pointer border border-slate-200 dark:border-slate-700"
                    >
              ...
              {/* Message Feed Area */}
              <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3.5 bg-slate-50/50 dark:bg-slate-950/60">
                {loadingMessages ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center text-slate-400 py-12">
                    <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800/80 flex items-center justify-center text-indigo-600 dark:text-indigo-400 mb-3">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 max-w-[85%] sm:max-w-[70%]">
                          ...
                          <div
                            className={`rounded-2xl px-4 py-2.5 text-sm shadow-xs transition-all ${
                              isDeleted
                                ? "bg-slate-100 dark:bg-slate-900 text-slate-400 dark:text-slate-500 italic border border-slate-200 dark:border-slate-800"
                                : isMe
                              ...
                              <div className="mt-2.5 space-y-2">
                                {msg.attachments.map((att: any) => {
                                  ...
                                  return (
                                    <div
                                      key={att.id}
                                      className="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-black/30 p-2"
                                    >
                                      {isImage ? (
                                        <a
              ...
              {typingUsers.length > 0 && (
                <div className="px-5 py-1.5 text-xs text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-950/30 flex items-center gap-1.5 border-t border-indigo-100 dark:border-slate-800/60">
              ...
              {attachments.length > 0 && (
                <div className="px-5 py-2 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60 flex items-center gap-2">
              ...
              <form
                ...
                className="p-3 md:p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center gap-2"
              >
                <input
                  type="file"
                  ref={fileInputRef}
