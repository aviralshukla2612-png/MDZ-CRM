...
export default function AttendanceWorkClockPage() {
  ...
  const [myTimeModRequests, setMyTimeModRequests] = useState<any[]>([]);
  ...
  const fetchTimeModRequests = async () => {
    try {
      ...
      const json = await res.json();
      if (json.success && Array.isArray(json.data)) {
        setMyTimeModRequests(json.data);
      }
    } catch (e) {
      console.error("fetchTimeModRequests error:", e);
    }
  };
...
function getDistanceInMeters(lat1: number, lon1: number, lat2: number, lon2: number) {
  ...
  return (
    <div className="space-y-8 pb-20 max-w-4xl mx-auto">
      {/* Module Header */}
      <PageHeader
        title="Millionaire Dizital Work Clock"
        ...
        actions={
          <button
            onClick={() => {
              setSelectedAttForMod(null);
              setIsTimeModOpen(true);
            }}
            className="px-3.5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-white font-bold text-xs shadow-sm transition-all flex items-center gap-2"
          >
            <Moon className="w-4 h-4" />
            <span>Appeal Late Hours / Time Mod</span>
          </button>
        }
      />
          ...
          <table className="w-full text-left text-xs">
            <thead className="text-slate-500 dark:text-slate-400 uppercase font-mono tracking-wider bg-slate-50 dark:bg-slate-900/50">
              <tr>
                ...
                <th className="p-3 font-semibold rounded-r-xl text-right">Action</th>
              </tr>
            </thead>
                  ...
                  return (
                    <tr key={record.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="p-3 font-medium text-slate-900 dark:text-slate-100 whitespace-nowrap">
                        ...
                        {record.punchOutReason?.includes("12:00 AM") && (
                      ...
                      <td className="p-3 text-right">
                        <button
                          type="button"
                          onClick={() => {
                            setSelectedAttForMod(record);
                            setIsTimeModOpen(true);
                          }}
                          className="px-2.5 py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/60 dark:hover:bg-indigo-900/60 text-indigo-600 dark:text-indigo-400 font-bold text-[11px] border border-indigo-200 dark:border-indigo-800 transition-colors inline-flex items-center gap-1"
                        >
                          <Edit3 className="w-3 h-3" />
                          <span>Appeal / Edit</span>
                        </button>
                      </td>
                    </tr>
                  );
      ...
      {/* MY TIME MODIFICATION REQUESTS / APPEALS */}
      {myTimeModRequests.length > 0 && (
        <div className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-slate-200/80 dark:border-slate-800/80 p-6 md:p-8 shadow-xl dark:shadow-2xl space-y-6">
          <div className="flex items-center justify-between border-b border-slate-200/80 dark:border-slate-800/80 pb-4">
            <div className="space-y-1">
              <h3 className="font-bold text-lg text-slate-900 dark:text-slate-100 flex items-center gap-2">
                <Moon className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                My Time Modification Appeals
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Status of appeals submitted for late shoots or overtime hours.
              </p>
            </div>
            <span className="text-xs font-mono font-bold px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20">
              {myTimeModRequests.length} REQUESTS
            </span>
          </div>

          <div className="space-y-3 text-xs">
            {myTimeModRequests.map((req) => (
              <div
                key={req.id}
                className="p-4 rounded-2xl bg-slate-50/80 dark:bg-slate-950/40 border border-slate-200/80 dark:border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {req.status === "PENDING" ? "⏳ Awaiting Sub/Admin" : req.status}
                    </span>
                  </div>
                  ...
                  {req.reviewNotes && (
                    <p className="text-indigo-600 dark:text-indigo-400 text-[10px] font-medium">
      ...
      {/* Time Modification / Appeal Modal */}
      <TimeModificationModal
        isOpen={isTimeModOpen}
        onClose={() => {
          setIsTimeModOpen(false);
          setSelectedAttForMod(null);
        }}
        onSuccess={() => {
          fetchAttendanceLogs();
        ...
        attendanceId={selectedAttForMod?.id}
